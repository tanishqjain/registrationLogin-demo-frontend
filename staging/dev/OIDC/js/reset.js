...
//loadCdcApiDynamically(cdcApiData, isStorageDomainOverride);
  ...
  try {
    ...
    if (cdcApiData.childApiKey) {
      cdcscripturl = `https://cdc.myaccount.electroluxgroup.eu/js/gigya.js?apiKey=3_MPoKZvKgqWRRUUENtIQ55e5FJbHQtJpVV9HN5UVoahm6ZjBpuIL658U6niR8oQm8&childApiKey=${cdcApiData.childApiKey}&lang=${language}`;
    } else {
      cdcscripturl =  `https://cdc.myaccount.electroluxgroup.eu/js/gigya.js?apiKey=3_MPoKZvKgqWRRUUENtIQ55e5FJbHQtJpVV9HN5UVoahm6ZjBpuIL658U6niR8oQm8&lang=${language}`
    }
  } catch (err) {
...
function afterSubmit(event){
	...
	if((event.screen == 'gigya-reset-password-screen')&& (event.response.errorCode == 0)){
        ...
        //window.location.href = cdcApiData.returnUrl;
            
            /*var backToLoginButton = document.getElementsByClassName("gigya-input-submit");
            
            for(i=0;i<backToLoginButton.length;i++){
            	
            	if(backToLoginButton[i].parentElement.classList.value.includes('backToLogin')){
                
                	document.getElementsByClassName("gigya-input-submit")[i].href = "https://www." + getCookie("location") + ".com/";
