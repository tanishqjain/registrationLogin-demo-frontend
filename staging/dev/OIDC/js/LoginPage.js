...
function createScreenSetParameters(cdcApiData){
	...
	const screenSetParams = {
		...
		//	startScreen: `gigya-${cdcApiData.render_screen}-screen-new`,
			startScreen: `gigya-${cdcApiData.render_screen}-signUp-screen`,
 ...
 */
 function onLoginHandler(eventObj) {
	...
	console.log("user" + eventObj.newUser);
	const screenSetParams = {
			screenSet: `${cdcApiData.spName}-RegistrationLogin`,
			startScreen: `gigya-login-complete`,
			context: cdcApiData.passReturnURL + "," + cdcApiData.copyUpdateParam,
			lang: language,
			containerID: "cdc-login-container"
		}
   ...
   setTimeout(() => console.log("Delay is over."), 7000);
   gigya.sso.continue();
   gigya.fidm.oidc.op.redirectToContinue({
    opKey: gigya.apiKey,
    context: gigya.getUrlParam("context"),
