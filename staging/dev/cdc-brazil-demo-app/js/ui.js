...
function setActiveBrandFlag(config) {
    /* SET CORRECT FLAG IN FUNCTION OF LANGUAGE */
    // Set language name from language code
    const brandName=config.raas_prefix;
    fetch("./config/brands.json")
        .then((res) => {
            return res.json();
        })
        .then((out) => {
            // Start demo page
            console.log("out : " , out)
            queryAll(".brand-dropdown .dropdown-content").forEach(
                (flagsContainer, i) => {
                    for (var j = 0; j < out.brands.length; j++) {
                        const oneBrand = out.brands[j];
                        const html = createBrandEntry(oneBrand);
                        flagsContainer.appendChild(html);
                    }
                }
            );
            ...
            // Tooltip
            queryAll(".flag-container").forEach((item, i) => {
                item.setAttribute("aria-label", brandName);
            });

            // Selected Flag
            const brandSelectedFlag = getFlagIconFor(out.brands, config.raas_prefix);
            queryAll(".flag-container >a>.brand-name").forEach((item) => {
                // Fix english flag.
                item.classList.add("brand-name-" + brandSelectedFlag);
            });
        })
        .catch((err) => {
            return console.error(err);
...
// Console DC Dropdown
// function setActiveConsoleDCFlag(config) {
//     const consoleDCName = config.console_prefix;
//     fetch("./config/console-dc.json")
//         .then((res) => {
//             return res.json();
//         })
//         .then((out) => {
//             // Start demo page
//             console.log("out : " , out)
//             queryAll(".console-dc-dropdown .dropdown-content-dc").forEach(
//                 (flagsContainer, i) => {
//                     for (var j = 0; j < out.consoleDC.length; j++) {
//                         const oneDC = out.consoleDC[j];
//                         const html = createConsoleDCEntry(oneDC);
//                         flagsContainer.appendChild(html);
//                     }
//                 }
//             );
...
//             // Tooltip
//             queryAll(".flag-container-console").forEach((item, i) => {
//                 item.setAttribute("aria-label", consoleDCName);
//             });

//             // Selected Flag
//             const dcSelectedFlag = getFlagIconFor(out.consoleDC, config.console_prefix);
//             queryAll(".flag-container-console >a>.console-dc-name").forEach((item) => {
//                 // Fix english flag.
//                 item.classList.add("console-dc-name-" + dcSelectedFlag);                
//             });
//         })
//         .catch((err) => {
//             return console.error(err);
...
function changeBrand(brand) {
    // set the language in local storage
    setInLocalStorage("raas_prefix", brand);

    // Refresh the page
    location.href = location.href;
}

function changeConsoleDC(consoleDC) {
    // set the language in local storage
    setInLocalStorage("console_prefix", consoleDC);
...
function getBrand() {
    // get the brand from local storage
    return getFromLocalStorage("raas_prefix");
}

function getConsoleDC() {
    // get the cosoleDC from local storage
    return getFromLocalStorage("console_prefix");
}
/**
...
function createBrandEntry(brand){
    // Add a
    var aObj = document.createElement("a");
    ...
    // Set attribute for img, such as id
    spanObj.setAttribute("class", "brand-name brand-name-" + brand.flag);
    spanObj.innerText = brand.label;

    // Create element and return
    aObj.onclick = function() {
        changeBrand(brand.key);
    };
    aObj.setAttribute("class", "dropdown-item");
    aObj.href = "#";
    aObj.appendChild(spanObj);
...
// function createConsoleDCEntry(consoleDC){
//     // Add a
//     var aObj = document.createElement("a");
...
//     // Set attribute for img, such as id
//     spanObj.setAttribute("class", "console-dc-name console-dc-" + consoleDC.flag);
//     spanObj.innerText = consoleDC.label;

//     // Create element and return
//     aObj.onclick = function() {
//         changeConsoleDC(consoleDC.key);
//     };
//     aObj.setAttribute("class", "dropdown-item");
//     aObj.href = "#";
//     aObj.appendChild(spanObj);
...
function toggleBrandDropDown(){
    queryAll(".brand-dropdown").forEach((brandDropdown, i) => {
        const isActive = brandDropdown.classList.contains("is-active");
        if (isActive) {
            brandDropdown.classList.remove("is-active");
        } else {
            brandDropdown.classList.add("is-active");
        }
    });
}
...
// function toggleConsoleDCDropDown(){
//     queryAll(".console-dc-dropdown").forEach((dcDropdown, i) => {
//         const isActive = dcDropdown.classList.contains("is-active");
//         if (isActive) {
//             dcDropdown.classList.remove("is-active");
//         } else {
//             dcDropdown.classList.add("is-active");
//         }
//     });
// }
...
function initChangeApiKeyModal() {
    ...
    console.log('Initializing change api key modal....');
