...
function loadConfigFromFile(out) {
    ...
    const storedBrand = getBrand();
    if (storedBrand !== null) {
        out.raas_prefix = storedBrand;
    }

    const storedConsoleDC = getConsoleDC();
    if (storedConsoleDC !== null) {
        out.console_prefix = storedConsoleDC;
    }
...
function initPage(user) {
    ...
    fetch(path)
        ...
        .then((out) => {
            ...
            setActiveBrandFlag(config);

            // 5. Get Console DC Dropdown
            // setActiveConsoleDCFlag(config);
