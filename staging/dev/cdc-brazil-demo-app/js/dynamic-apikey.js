...
function changeAPIKey() {
    ...
    const consoleLoaclDCVal = document.querySelector(".console-dc-input").value;
    const consoleDC = consoleLoaclDCVal === "" || consoleLoaclDCVal === null ? config.console_prefix : consoleLoaclDCVal;
...
function loadGigyaForApiKey(apiKey) {
    ...
    const consoleDCVal = config.console_prefix;
    // const consoleLoaclDCVal = document.querySelector(".console-dc-input") !== null ? document.querySelector(".console-dc-input").value : "";
    // const consoleDCVal = consoleLoaclDCVal === "" || consoleLoaclDCVal == null ? config.console_prefix : consoleLoaclDCVal;
    log("2. Load Gigya File with apiKey " + apiKey, "LOAD GIGYA FILE");
    // alert(config.console_prefix);
    var newScript = document.createElement("script");
    var scriptUrl = "";
    if(consoleDCVal === 'eu1') scriptUrl = "https://cdns.gigya.com/js/gigya.js?apikey=" + apiKey;
    else scriptUrl = "https://cdns."+consoleDCVal+".gigya.com/js/gigya.js?apikey=" + apiKey;
...
function clearCustomApiKey() {
    ...
    localStorage.removeItem("console_prefix");
    window.location.href = window.location.href;
}
...
function validateAPIKey(apiKey) {
    ...
    const validateAPIKeyUrl = "https://accounts."+consoleDCVal+".gigya.com/accounts.getJWTPublicKey?apikey="+apiKey;
    console.log("validateAPIKeyUrl : " , validateAPIKeyUrl);
    ...
    console.log("request response : ",request.status + request.statusText )
    if (request.status === 200) {
    //    validApiKeyResponse = request.responseText;
    var responseData = {};
    responseData = JSON.parse(request.responseText);

    if(responseData.statusCode == '400' || responseData.statusCode == '403' || responseData.statusCode == '404'){
        validApiKeyResponse = responseData.errorDetails;
    }else{
        validApiKeyResponse = request.statusText;
        // changeConsoleDC(consoleDCVal);
    } 
        ...
        validApiKeyResponse = "Either API Key or DC is Invalid";
    }
