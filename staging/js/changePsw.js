...
/**
 * Start function to get a Brandsite Change Password Frame
 * 
 * Gets a change password CDC frame (I-frame) from the 
 * CDC API.
 * 
 ...
 * change password CDC frame.
 */
function getCdcChangePasswordFrame() {
  try {
    ...
    centerErrorContent()
    showErrorContent();
    const header = 'Error when creating change password page';
...
function getCdcApiData(cdcApiData, urlSearchParamsObj) {
  try {
    ...
    cdcApiData.language = getLanguageFromChangePswUrl(urlSearchParamsObj);
 ...
 * Gets an change password i-frame from the CDC API and
 * add it the HTML tag with the id of
 * cdc-change-psw-container
 */
...
function createScreenSetParameters(cdcApiData) {
  try {
    const screenSetParams = {
      screenSet: `${cdcApiData.brand}-ProfileUpdate`,
      startScreen: "brandsite-change-password-screen",
      containerID: "cdc-change-psw-container",
...
function afterScreenLoad(event) {
  ...
  if (event.currentScreen == 'brandsite-change-password-confirmation-screen') {
    console.log('CALLING AFTER SCREEN LOAD FOR SUCCESS SCREEN');
    var ua = navigator.userAgent.toLowerCase(); 
    if (ua.indexOf('safari') != -1 && ua.indexOf('chrome') == -1) { 
     hideSubmitButtonForSafari();
    }
  } else {
...
/**
 * Hides the Close button in the screen set for Safari Browser.
 */
 function hideSubmitButtonForSafari() {
